...
Most recent recommendation systems rely on a mix of both methods. The two algorithms introduced here are largely based on collaborative filtering since no information is available about the users, other than their ratings. Only two complementary information are available about the movies: the genre of the movie, and the year of release of the movie.
...
* Section **3. `r I(sec_two)`** provides a detailed account of the data preparation process. It also analyses and visualises all the features in the dataset.
...
The general purpose of statistical or machine learning is to provide an estimate for the relationship $f$ between an outcome $Y$ and a set of predictors $X_1,\cdots,X_p$ defined such that:
$${Y} = f(X_1,\cdots,X_p) + \epsilon_i$$
$\epsilon_i$ is the difference, or error, between $f(X_1,\cdots,X_p)$ and the real value $Y$. It is due to the fact that the predictors $X_1,\cdots,X_p$ might not suffice to calculate exactly $Y$.

Knowing $\hat f$ (the notation $\hat f$ is used to denote an estimate of $f$) is useful for two reasons. It can be used to understand better how the predictors influence the outcome (*inference*). It can also be used to predict values of $Y$ from the predictors (*prediction*). Once an estimate for $f$ is available, $\hat Y$, an estimate of $Y$, can be calculated:
$$\hat{Y} = \hat f(X_1,\cdots,X_p)$$
$$Y = \hat Y + \epsilon$$
$\epsilon$ is the difference, or error, between the estimate $\hat{Y}$ and the real value $Y$. It is made of two parts: the *reducible* error ($\epsilon_r$), and the *irreducible* error ($\epsilon_i$).
$$\epsilon = \epsilon_i + \epsilon_r$$
$\epsilon_i$ cannot be reduced because $Y$ might be determined by unknown predictors other than $X_1,\cdots,X_p$. Even if and when $f$ = $\hat f$, it is not equal to 0.  
...
All the work of developing a machine learning model consists in identifying $\hat f$ in such a way that the error $\epsilon$ is as small as possible. The question therefore becomes: what shape should $f$ take?  

In this tutorial two models are developed. They are detailed next. 
...
Let's start with downloading and preparing the data. Running each of these blocks is somewhat time consuming. For this reason some important datasets (`movielens`, `train_set`, `test_set`, `validation`) will be saved for future use in order to save time.
...
```{r Load local datasets if any, echo=FALSE, results='hide', eval=local_data}
...
```
The **golden rule of machine learning** is that the validation data should *never* be used to fit or train the models in any way. Validation data should only be used to provide an unbiased evaluation of a model, close to how the models would perform on unknown data. If learnings from validation data were included in the model, this would not be possible. Only the `train_set`, `test_set` and `train_test` (which includes both train and test sets) can be used to explore the data, make model choices, refine parameters and fit the models.
...
```{r Start data exploration and visualisation, results=FALSE}
...
* `movieId`: a numerical which identifies the movie the rating refer to,  

* `rating`: a numerical, the rating from `userId` on `movieId`,  

* `timestamp`: an integer which contains information about the date and time of the rating,  

* `title`: a character string, which contains the name of the movie. This column is extracted from the column `ttyr` which is kept for verification purposes,  

* `year`: an integer, the year of the release data of the movie,  

* `genres`: a character string, which list all the genres of the movie.  

Now let's see how many ratings, *unique* movies and *unique* users there are in the dataset:
```{r Number of unique movies and users}
...
What is the mean and median values of the ratings? The mean is lower than the median, meaning that the distribution is skewed to the left. Users give on average ratings of 3.5, with as many users giving a rating of 4 than giving a rating below 4.
```{r Mean and median}
...
```{r Ratings per user and per movie, warning=FALSE, message=FALSE}
...
```{r Nb ratings for the 10 least rated movies, message=FALSE}
...
```{r Histogram of number of ratings per movie, fig.align='center', message=FALSE}
...
```{r Histogram of number of ratings per user,  fig.align='center', message=FALSE}
...
```{r Max number of rating per user}
max(user_ratings$n_ratings)
```

### Distribution of rating averages per user and movie

Now let's look at the distribution of the rating average per movie.
```{r Density of movie averages,  fig.align='center'}
...
As the density plot shows, lots of movies have an average between 3 and 4.  

Do movies with higher average have more ratings? Indeed, it seems that the higher a movie average, the higher the number of ratings.
```{r Number of ratings vs. movie average,  fig.align='center', message=FALSE}
...
```{r Distribution of user averages,  fig.align='center'}
...
As this density plot shows, a large proportion of the users give ratings of 3.5 and above. Let's have a look at the means of user averages and movie averages. The results show that the mean of user averages is higher than the mean of movie averages, which is explained by the fact that movies with higher ratings get rated more.
```{r Means of user averages and movie averages}
...
### Exploring the genres, year, and time stamps

The previous section has provided great insights about the distribution of ratings and rating averages per users and movies. In this section the genres, year and time stamps of the ratings and movies are analysed.

#### Genres

Let's start with the genre of the 10 most rated movies.
```{r Genre of the 10 most rated movies, message=FALSE}
...
```
  ...
  As the table shows, some movies have more than one genre. Let's see how many different groupings of genres exist.
```{r Distinct genre groupings}
n_distinct(train_test$genres)
```
  
  Let's see what are the 10 most common genre combinations.
```{r 10 most common genres combinations}
...
```{r Extraction and counting of unique genres}
list_genres <- unique(unlist(strsplit(train_test$genres, split = "\\|")))
print(list_genres)
print(length(list_genres))
```
There are 19 different unique genres. Some movies have no genre listed, and there is a total of 797 different combination of genres.

#### Year of release

Let's look at the number of movie releases per year.
```{r Histogram of the number of movie releases per year, fig.align='center'}
...
```
   ...
   Most of the releases have taken place after 1995. The last movie release in the dataset is `r I(max(year_movie$year))`. Old movies make only a small part of the dataset.
   
  What is the number of ratings per release year? As the histogram below shows, few ratings are about old movies. This can both mean that the proportion of old movies in the dataset is low and that viewers are less interested in these old movies.
```{r Number of ratings vs. year of release, message = FALSE, fig.align='center'}
...
```{r 10 release years with most ratings, message=FALSE}
...
The `timestamp` value gives the time and date when a rating was made. Let's have a look at how ratings have changed with time.  
In what year where the youngest and oldest rating made? 
```{r Oldest and youngest ratings}
...
```
How has the number of ratings per year evolved over time? Year 2000 has seen the most ratings.
```{r Number of ratings per year, message = FALSE, fig.align='center'}
...
```{r Results data frame and updating fuction}
...
Let's model one bias after another using regularisation and check, each time, what is the corresponding RMSE. This will give a good sense of how each bias contributes to the accuracy of the model.

### Naive mean (base model)

Let's start with a very simple algorithm, `naive_mean` which consist in taking the average rating of the training dataset and we apply it to the test set. As the next table shows, the performance of this base model is low. Hopefully adding the different biases will help improve this performance.

```{r Base model - Naive mean, message=FALSE, warning=FALSE}
...
```{r Movie effect (regularised), message=FALSE, warning=FALSE}
model_nb <- 2
...
```{r User effect (Regularised), message=FALSE, warning=FALSE}
model_nb <- 3
# Define a sequence of penalisation factor (lambda) for the regularization.
lambda <- seq(3, 7, 0.5)
...
```{r Release date effect (regularised), message=FALSE, warning=FALSE}
model_nb <- 5
# Define a sequence of penalisation factor (lambda) for the regularization.
lambda <- seq(0, 10, 2)
...
Next the regularised date (`timestamp`) of the ratings is added to the model. In this case, we need to regroup the time stamps per period (`week`, `month` or `year`) in order to compute the regularised average.
For each period, the code below calculates the difference between the period average and the overall average corrected by the regularized movie, user, genre and year averages (mu, bm, bu, bg, by). This difference is the date bias or effect $b_t$ of the model.
The code below run with a weekly period. Running it for a month or year period would show that the `week` period has the most impact.

```{r Date of rating effect (regularised), message=FALSE, warning=FALSE}
model_nb <- 6
...
By adding successively different biases or effects in such a way that all the information available in the dataset was used, the accuracy of the model has been improved. Some biases have more impact on the accuracy of the model than others. The biases with the most impact are the movie and user biases (more than 18% improvement).
...
To operate, the `recosystem` requires as input a 3-columns matrix corresponding to the users, movies, and rating. Let's build this. The function `data_memory()` is used to provide input to the recommender system. Help on how to use this function is available using the command `?recosystem::data_memory` in the R console.

```{r Matrix factorisation - create train and test matries}
...
Next the model object must be created using the function `Reco()` which returns a class RecoSys object which makes the methods `$train()`, `$tune()` and `$predict()` accessible. Help on this function is available using the command `?recosystem::Reco`.
```{r Create RecoSys object}
reco_obj <- Reco()
```
It is now time to train the model with the method `$train()`. Helps on this function (`?recosystem::train`) indicates that the methods uses as input arguments:
...
For the first attempt, the default options `opts` are used, except for the number of thread, which is set at 4 to increase computing speed, and the number of iteration, `niter`, sets at 10 instead of the default value of 20.

```{r Train with 10 iterations}
...
```{r, eval=local_data, echo=FALSE, include=FALSE}
# If local_data this block is executed and the tuning parameters 
# reco-opts.rds gets loaded and used.
opts <- readRDS("data/reco-opts.rds")
```

Now before training the model, let's use the `$tune` method to try and improve the accuracy of the model. For detailed information about the parameters of the function, type `?recosystem::tune`.
This step takes time to compute, so be patient.

```{r Run tunning function, eval=!local_data, message=FALSE, warning=FALSE}
...
```{r Train model with optimised parameters, message=FALSE, warning=FALSE}
model_nb <- 9
...
This section presents the results and discusses the performance of the models. The **validation set** is now used to estimate these performances. Because the set has not been used so far, these performances can be considered an unbiased estimate of the performance of the models on new data.
...
```{r RMSE Linear Model applied to validation set, message=FALSE, warning=FALSE}
...
```{r RMSE Matrix Factorisation model applied to validation set,  message=FALSE, warning=FALSE}
...
In this tutorial, the typical workflow of a data-science project has been introduced and followed. Two models have been developed, trained, tuned and applied to the publicly available "MovieLens 10M" dataset.  
